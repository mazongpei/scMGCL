...
def parse_arguments():
    ...
    parser.add_argument('--cluster_num', type=int, default=-1, help='Number of clusters')#这个也是必须的
    parser.add_argument('--input_h5ad_path', type=str, default="", help='path to input h5ad file')#这个是必须要写的
...
def main():
    ...
    for epoch in range(0, args.epochs):
        ...
        for i, (matrix, index, label) in enumerate(train_loader):
            ...
            loss = gamma*(args.alpha * loss1 + (1 - args.alpha) * loss2 + args.beta * loss3) + delta*loss_cluster + (1-delta)*loss_graph
