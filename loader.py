...
class Matrix(Dataset):
    def __init__(self,
        ...
        # 深拷贝数据
        self.data_for_augmentation = deepcopy(self.data)#data_for_augmentation 和data是一模一样的数据用于数据增强
    ...
    def RandomAugmentation(self, sample, index):
        ...
        with warnings.catch_warnings():
            ...
            for neighbor in NeighborLoader(self.global_graph, num_neighbors=[self.data.shape[0]],
                                           input_nodes=torch.tensor([index], dtype=torch.long)):
